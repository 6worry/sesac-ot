...
export function generateitemType() {
    ...
    return `${selectedProduct}, ${product_type[0]}`;
    }

export function generateitemPrice() {
    const product = ['아메리카노', '카페라떼', '카페모카', '에스프레소', '토피넛라떼', '카라멜 마끼아또', '민트모카', '딸기에이드', '히비스커스', '캐모마일', '얼그레이', '생과일주스', '프라푸치노', '레몬에이드', '당근케잌', '초코케잌', '크로플', '소금빵', '초코머핀', '프레즐'];
    ...
    const selectedProduct1 = product[Math.floor(Math.random() * product.length)];
    return productPrices[selectedProduct1] || '가격 정보 없음';
}
